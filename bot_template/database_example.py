    ...
    def get_bot_state(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "tp3_executed": bool(result[8]),
                "current_pnl_usd": float(result[9]) if result[9] else None,
                "current_pnl_percent": float(result[10]) if result[10] else None,
                "regime": result[11],
                "probability": float(result[12]) if result[12] else None,
                "volatility": float(result[13]) if result[13] else None,
                "threshold_used": float(result[14]) if result[14] else None,
                "available_capital": float(result[15]) if result[15] else None,
                "stop_loss_price": float(result[16]) if result[16] else None,
                "stop_loss_percent": float(result[17]) if result[17] else None,
                "tp1_price": float(result[18]) if result[18] else None,
                "tp1_percent": float(result[19]) if result[19] else None,
                "tp2_price": float(result[20]) if result[20] else None,
                "tp2_percent": float(result[21]) if result[21] else None,
                "tp3_price": float(result[22]) if result[22] else None,
                "tp3_percent": float(result[23]) if result[23] else None,
                "total_pnl": float(result[24]) if result[24] else None,
                "total_pnl_percent": float(result[25]) if result[25] else None,
                "total_trades": int(result[26]) if result[26] else 0,
                "winning_trades": int(result[27]) if result[27] else 0,
                "trades_blocked": int(result[28]) if result[28] else 0,
                "last_check": result[29],
                "last_signal_action": result[30]
            }
